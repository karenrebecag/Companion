...
/// Implements <code>ArtboardListener</code> to receive callbacks from the command queue. Manages requests
...
/// All continuation-based methods are wrapped with <code>withTaskCancellationHandler</code> because
/// <code>withCheckedThrowingContinuation</code> does not auto-resume on task cancellation. Without
...
- (void)onStateMachineInstantiated:(uint64_t)artboardHandle requestID:(uint64_t)requestID stateMachineHandle:(uint64_t)stateMachineHandle;
/// Called when artboard volume information is received.
/// Listener callback invoked by the command server. Dispatches to main actor to access
/// requests dictionary and resume its continuation with the volume value.
- (void)onArtboardVolumeReceived:(uint64_t)artboardHandle requestID:(uint64_t)requestID volume:(float)volume;
...
/// \param fileHandle The file handle associated with the failed operation
///
/// \param requestID The request ID matching the one used when initiating the operation
///
...
- (void)onArtboardInstantiated:(uint64_t)fileHandle requestID:(uint64_t)requestID artboardHandle:(uint64_t)artboardHandle;
...
- (void)onViewModelInstanceInstantiated:(uint64_t)fileHandle requestID:(uint64_t)requestID viewModelInstanceHandle:(uint64_t)viewModelInstanceHandle;
...
SWIFT_ENUM_FWD_DECL(NSInteger, RiveFallbackFontDescriptorDesign)
SWIFT_ENUM_FWD_DECL(NSInteger, RiveFallbackFontDescriptorWeight)
SWIFT_ENUM_FWD_DECL(NSInteger, RiveFallbackFontDescriptorWidth)
...
+ (void)logCancelledAssetDownload:(RiveFileAsset * _Nonnull)asset fromURL:(NSURL * _Nonnull)url;
...
SWIFT_ENUM_FWD_DECL(NSInteger, RiveTouchEvent)
@class RiveEvent;
...
@interface RiveUIView : NSView <MTKViewDelegate>
...
/// \param preferredFramesPerSecond Integer number of seconds to set preferred FPS at
///
- (void)setPreferredFramesPerSecond:(NSInteger)preferredFramesPerSecond SWIFT_AVAILABILITY(macos,introduced=14);
...
- (void)onSemanticsDiffReceived:(uint64_t)stateMachineHandle requestID:(uint64_t)requestID diff:(RiveSemanticsDiff * _Nonnull)diff;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
- (void)onViewModelInstanceViewModelNameReceived:(uint64_t)viewModelInstanceHandle requestID:(uint64_t)requestID viewModelName:(NSString * _Nonnull)viewModelName;
...
- (void)onViewModelInstanceNameReceived:(uint64_t)viewModelInstanceHandle requestID:(uint64_t)requestID name:(NSString * _Nonnull)name;
...
SWIFT_CLASS_NAMED("_RiveMainActor")
@interface _RiveMainActor : NSObject
+ (void)assertIsolated:(NSString * _Nonnull)message;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
